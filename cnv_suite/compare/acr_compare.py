...
def acr_compare(
    file_1: pd.DataFrame | PathLike | None = None, file_2: pd.DataFrame | PathLike | None = None, fit_params=False
):
    """
    ...
    if fit_params:
        bins["mu.major_1"] = bins.apply(
            lambda x: optimal_scale_factor * (optimal_purity * (x["mu.major_1"] - 1) + 1), axis=1
        )
        bins["mu.minor_1"] = bins.apply(
            lambda x: optimal_scale_factor * (optimal_purity * (x["mu.minor_1"] - 1) + 1), axis=1
        )
...
def get_avg_overlap(bins, params=None):
    ...
    major_overlap = bins.apply(
        lambda x: calc_overlap(
            x["mu.major_1"], x["sigma.major_1"], x["mu.major_2"], x["sigma.major_2"], x["unique"], params=params
        ),
        axis=1,
    )
    minor_overlap = bins.apply(
        lambda x: calc_overlap(
            x["mu.minor_1"], x["sigma.minor_1"], x["mu.minor_2"], x["sigma.minor_2"], x["unique"], params=params
        ),
        axis=1,
    )
    ...
    overlap_score = np.average(
        np.concatenate((major_overlap, minor_overlap)), weights=np.concatenate([bins["length"], bins["length"]])
    )
    return overlap_score, major_overlap, minor_overlap


def calc_overlap(
    mu1: float, sigma1: float, mu2: float, sigma2: float, unique: bool, params: np.ndarray | None = np.array([1.0, 1.0])
):
    """
    ...
    mid_section1 = NormalDist(mu1, sigma1).cdf(max(x_intersect)) - NormalDist(mu1, sigma1).cdf(min(x_intersect))
    mid_section2 = NormalDist(mu2, sigma2).cdf(max(x_intersect)) - NormalDist(mu2, sigma2).cdf(min(x_intersect))
...
def _union_one_sided(seg1_df: pd.DataFrame, seg2_df: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    for chrom in np.arange(1, 23):  # assumes segments in each chromosome (except for XY)
        ...
        for i, (start, end) in enumerate(zip(segments1["Start.bp"], segments1["End.bp"])):
            # create bins for this segment1, updating pointer2 to save computation
            bin_list, pointer2 = create_bins(
                start, end, segments2, pointer2, segments1.loc[i][STAT_COLUMNS], chrom, bin_list=bin_list
            )
...
def compute_aad(target: pd.DataFrame, pred: pd.DataFrame) -> float:
    ...
    for row in pred.to_numpy():
        src_by_chr[row[src_chr]].append(Segment(row[src_start], row[src_end], row[src_minor], row[src_major]))
    for row in target.to_numpy():
        tar_by_chr[row[tar_chr]].append(Segment(row[tar_start], row[tar_end], row[tar_minor], row[tar_major]))
    ...
    for chr in src_by_chr.keys() & tar_by_chr.keys():
        ...
        while src_idx < len(src_segments) and tar_idx < len(tar_segments):
            ...
            start = max(src_segments[src_idx].start, tar_segments[tar_idx].start)
    ...
    params = minimize(lambda p: np.sum(weights * np.abs(src * p[0] + p[1] - tar)), np.array([1, 0])).x
    return np.sum(weights * np.abs(src * params[0] + params[1] - tar))


# TESTING
def compare_outputs(src_folder: PathLike, solution_folder: PathLike) -> dict[float, list[float]]:
    """
    ...
    simulated_problems = glob.glob(f"{src_folder}/**/sim_profile.tsv", recursive=True)

    res = {}

    for problem_path in simulated_problems:
        rel_path = "\\".join(problem_path.replace(str(src_folder), "").split("\\")[:-1])
        ...
        print(compute_aad(sol_df, pred_df))
...
def main():
    ...
    compare_outputs(
        r"C:\Users\rsolan\Documents\hap-rs\data\mass\ch1022gl", r"C:\Users\rsolan\Documents\hap-rs\out\mass\ch1022gl"
    )
