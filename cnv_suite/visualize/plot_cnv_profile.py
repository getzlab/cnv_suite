...
from utils import calc_cn_levels, calc_absolute_cn, calc_avg_cn
...
def plot_acr_static(
    ...
    ax.hlines(
        seg_df["mu_minor"].values,
        seg_df["genome_start"],
        seg_df["genome_end"],
        color=seg_df["color_bottom"],
        lw=min_seg_lw,
    )
    ax.hlines(
        seg_df["mu_major"].values,
        seg_df["genome_start"],
        seg_df["genome_end"],
        color=seg_df["color_top"],
        lw=min_seg_lw,
    )
...
def plot_acr_subplots(fig_list, title, fig_names, csize, height_per_sample=350, **kwargs):
    ...
    fig = plotly.subplots.make_subplots(
        ...
        cols=1,
        shared_xaxes=True,
        subplot_titles=fig_names,
        vertical_spacing=0.15 / len(fig_list),
        **kwargs,
    )
    ...
    fig.update_layout(
        height=len(fig_list) * height_per_sample + 50, title_text=title, title_font_size=26, plot_bgcolor="white"
    )
...
def plot_acr_interactive(
    ...
    if purity and ploidy:
        c_0, c_delta = calc_cn_levels(
            purity, ploidy, avg_cn=calc_avg_cn(seg_df, allele_col="mu_minor", total_cn_col="tau")
        )
        ...
        seg_df["color_bottom_diff_adj"], seg_df["color_top_diff_adj"] = calc_color(
            seg_df, "mu_major_adj", "mu_minor_adj"
        )
    ...
    update_cnv_color_absolute(
        fig, seg_df, absolute=False, color=segment_colors, start_trace=trace_num, end_trace=trace_end
    )
...
def update_cnv_scatter_cn(fig, major, minor, sigma, start_trace, end_trace, lw=None):
    ...
    for i, (minor_val, major_val, sigma) in enumerate(zip(minor, major, sigma)):
        fig.data[start_trace + 4 * i]["y"] = [
            minor_val + sigma,
            minor_val - sigma,
            minor_val - sigma,
            minor_val + sigma,
        ]
        fig.data[start_trace + 4 * i + 1]["y"] = [
            major_val + sigma,
            major_val - sigma,
            major_val - sigma,
            major_val + sigma,
    ...
    for chrom in chr_order:
        if type(ax) == go.Figure:
            if is_plotly_figure:
                ax.add_shape(
                    x0=base_start,
                    x1=base_start + csize[chrom],
                    y0=0,
                    y1=1,
                    fillcolor=patch_color,
                    opacity=0.1,
                    layer="below",
                    line_width=0,
                    yref="y domain",
            ...
            p = patches.Rectangle(
                (base_start, -0.2), csize[chrom], height, fill=True, facecolor=patch_color, edgecolor=None, alpha=0.1
            )  # Background
...
def calc_color(seg_df, major, minor):
    ...
    color_bottom = seg_df.apply(
        lambda x: colors.rgb2hex(cmap(int(np.floor(max(0, (0.5 - 0.5 * scale_diff(x[major] - x[minor])) * 255))))),
        axis=1,
    )
    color_top = seg_df.apply(
        lambda x: colors.rgb2hex(cmap(int(np.floor(min(255, (0.5 + 0.5 * scale_diff(x[major] - x[minor])) * 255))))),
        axis=1,
    )
...
def prepare_df(df, csize, suffix=".bp"):
    ...
    if "mu.major" in df.columns:
        col_names = dict(
            mu_major="mu.major", mu_minor="mu.minor", sigma_major="sigma.major", sigma_minor="sigma.minor", tau="tau"
        )
    ...
    chrom_start = {
        chrom: start
        for (chrom, start) in zip(np.append(chr_order, "Z"), np.cumsum([0] + [csize[a] for a in chr_order]))
    }
...
def save_static_plot(
    seg_df, output_fn, csize=None, segment_colors=None, sigmas=None, min_seg_lw=None, y_upper_lim=None
):
    if not csize:
...
def main():
    ...
    parser.add_argument(
        "-sc",
        "--segment_colors",
        choices=["black", "difference", "cluster", "blue_red"],
    ...
    parser.add_argument(
        "--min_seg_lw", help="Segment line_width (all segments if sigmas are hidden else minimum); default=2"
    )
    ...
    if args.csize_file and os.path.exists(args.csize_file):
        ...
        if ext == ".bed":
            ...
            columns = ["chr", "len"]
        csize = (
            pd.read_csv(args.csize_file, sep="\t", header=None, names=columns)
            .set_index("chr", inplace=True)
            .to_dict()["len"]
        )
    else:
        csize = None
    ...
    save_static_plot(
        ...
        args.output_fn,
        csize=csize,
        segment_colors=args.segment_colors,
        sigmas=args.sigmas,
        min_seg_lw=args.min_seg_lw,
        y_upper_lim=args.y_upper,
    )
